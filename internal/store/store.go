...
func (s *Store) migrate(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS tasks (
			...
			super_mode_base TEXT NOT NULL DEFAULT '',
	...
	if err := s.ensureColumn(ctx, "tasks", "super_mode_base", "TEXT NOT NULL DEFAULT ''"); err != nil {
		return err
	}
...
func (s *Store) ListTasks(ctx context.Context) ([]model.Task, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT
			t.id, t.name, t.account_id, COALESCE(a.name, ''), t.proxy_group_id, COALESCE(pg.name, ''), t.proxy_mode, t.super_mode, t.super_mode_base,
...
func (s *Store) CreateTask(ctx context.Context, input model.TaskInput) (model.Task, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		INSERT INTO tasks (
			name, account_id, proxy_group_id, proxy_mode, super_mode, super_mode_base, project_id, project_name, screen_id, sku_id,
...
func (s *Store) UpdateTask(ctx context.Context, id int64, input model.TaskInput) (model.Task, error) {
	...
	_, err = s.db.ExecContext(ctx, `
		UPDATE tasks
		SET name = ?, account_id = ?, proxy_group_id = ?, proxy_mode = ?, super_mode = ?, super_mode_base = ?, project_id = ?, project_name = ?, screen_id = ?, sku_id = ?,
			session_name = ?, ticket_level = ?, ticket_display = ?, ticket_price = ?,
			sale_start = ?, sale_status = ?, link_id = ?, is_hot_project = ?,
			task_mode = ?, duration_mode = ?, selected_tickets = ?, rush_duration_seconds = ?,
			order_type = ?, pay_money = ?, buyer_info = ?, buyer = ?, tel = ?, deliver_info = ?, phone = ?,
...
func scanTask(scanner taskScanner, task *model.Task) error {
	...
	task.SuperModeBase = model.NormalizeSuperModeBase(task.SuperModeBase)
	if task.SuperModeBase == "" {
		task.SuperModeBase = model.SuperModeBaseShow
	}
...
func normalizeTaskInput(input model.TaskInput) model.TaskInput {
	...
	input.SuperModeBase = model.NormalizeSuperModeBase(input.SuperModeBase)
